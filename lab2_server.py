...
#function that takes in a connection from client and then
#calls the send intro message function
#calls the receive long message function, and then stores and prints the results
def send_recv_msgs(x):

    #for testing
    print("running thread function")

    # Send the introduction message by implementing `send_intro_message` above.
    send_intro_message(x)

    # Receive long message by implementing the function above
    message = receive_long_message(x)
...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        while True:
            ...
            #initializes thread
            thread = Thread(target = send_recv_msgs, args=[conn])
